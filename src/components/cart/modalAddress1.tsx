...
export const ModalAddress = ({
    ...
    const { setAddressClient, setLocation } = useCheckoutForm();
    ...
    useEffect(() => {
        if (isOpen && addressToEdit) {
            ...
            updateField("selectedType", addressToEdit.propertyType || "house");
        }
    ...
    const handleAuthenticatedUser = async () => {
        try {
            ...
            const location = await LocationService.addLocation(token, createLocationData());
            ...
            const delivery = await AddressService.createDelivery(location.body.id);
            const deliveryData = delivery.body.delivery;
            const kitchen = delivery.body.kitchen;
            console.log('delivery creado segun location :', delivery);
            //creamos el delivery segun la location creada

            console.log('Ubicación creada con éxito:', location.body);
            ...
            console.log('address to set in checkout form :', rta);
